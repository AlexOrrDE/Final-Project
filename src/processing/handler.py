...
import io
import pandas as pd
from write_to_s3_processing import write_to_s3_processing
...
function_dict = {"counterparty" : create_dim_counterparty, "currency" : create_dim_currency, "date" : create_dim_date, "design" : create_dim_design, "address" : create_dim_location, "staff" : create_dim_staff, "sales_order" : create_fact_sales_order}
...
def processing_handler(event, context):
    try:
        ...
        bucket = s3_resource.Bucket("totesys-test")
        ...
        if "dim_date" not in check_dim_date:
            ...
            write_to_s3_processing("dim_date", returned_parquet)
        ...
        for table_name in event:
            ...
            if table_name in function_dict:
                ...
                write_to_s3_processing(key, returned_parquet)
